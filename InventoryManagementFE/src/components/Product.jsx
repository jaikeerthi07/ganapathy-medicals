...
export default function ItemsPage() {
  ...
  const getExpiryStatus = (expiryDate) => {
    if (!expiryDate) return { color: 'inherit', label: '' };
    const expiry = new Date(expiryDate);
    const today = new Date();
    const diffTime = expiry - today;
    const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));

    if (diffDays < 0) return { color: '#ef4444', label: '(EXPIRED)', weight: 'bold' };
    if (diffDays < 30) return { color: '#f59e0b', label: '(Expiring Soon)', weight: 'bold' };
  ...
  const handleEditSave = async () => {
    ...
    try {
      ...
      const productData = {
        ...
        batchNo: editingItem.batchNo?.trim() || "",
        expiryDate: editingItem.expiryDate?.trim() || "",
        manufacturer: editingItem.manufacturer?.trim() || "",
        hsnCode: editingItem.hsnCode?.trim() || "",
        gstRate: parseFloat(editingItem.gstRate) || 0,
        category: editingItem.category?.trim() || "",
        composition: editingItem.composition?.trim() || "",
          ...
          <div style={modalStyles.content}>
            <div style={modalStyles.modalHeader}>
              <h2 style={modalStyles.modalTitle}>
                ...
                {editingItem.isNew ? 'Add New Medicine' : 'Edit Medicine Details'}
              </h2>
              <button 
            ...
            <div style={modalStyles.formGroup}>
              <label style={modalStyles.label}>Medicine Name *</label>
              <input
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
              <div style={modalStyles.formGroup}>
                <label style={modalStyles.label}>Batch Number *</label>
                <input
                  style={modalStyles.input}
                  value={editingItem.batchNo || ""}
                  onChange={(e) => handleEditChange("batchNo", e.target.value)}
                  placeholder="e.g. AM12345"
                />
              </div>
              <div style={modalStyles.formGroup}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
              <div style={modalStyles.formGroup}>
                <label style={modalStyles.label}>Strength/Dose</label>
                <input
                  style={modalStyles.input}
              ...
              <div style={modalStyles.formGroup}>
                <label style={modalStyles.label}>Medicine Form</label>
                <select
                  style={modalStyles.input}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
              <div style={modalStyles.formGroup}>
                <label style={modalStyles.label}>Manufacturer</label>
                <input
                  style={modalStyles.input}
            ...
            {/* <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
              ...
              <div style={modalStyles.formGroup}>
                <label style={modalStyles.label}>Shelf Number</label>
                <input
                  style={modalStyles.input}
                  value={editingItem.shelfNo || ""}
                  onChange={(e) => handleEditChange("shelfNo", e.target.value)}
                  placeholder="e.g. S-5"
                />
              </div>
            </div> */}
            ...
            {/* <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
              <div style={modalStyles.formGroup}>
                ...
                <select
                  ...
                  onChange={(e) => handleEditChange("storageConditions", e.target.value)}
                >
                  <option value="">Select Condition</option>
                  <option value="Normal">Normal</option>
                  <option value="Cool Place (2-8°C)">Cool Place (2-8°C)</option>
                  <option value="Away from sunlight">Away from sunlight</option>
                  <option value="Keep Dry">Keep Dry</option>
                </select>
              </div>
              <div style={modalStyles.formGroup}>
                <label style={modalStyles.label}>Reorder Level</label>
                <input
                  style={modalStyles.input}
                  type="number"
            ...
            <div style={modalStyles.formGroup}>
              <label style={{...modalStyles.label, display: 'flex', alignItems: 'center', cursor: 'pointer'}}>
                <input
                  ...
                  checked={editingItem.isPrescriptionRequired || false}
                  onChange={(e) => handleEditChange("isPrescriptionRequired", e.target.checked)}
                />
            ...
            <div style={modalStyles.formGroup}>
              <label style={modalStyles.label}>Composition / Generic Name</label>
              <textarea
                style={{...modalStyles.input, minHeight: '60px'}}
                value={editingItem.composition || ""}
                onChange={(e) => handleEditChange("composition", e.target.value)}
                  ...
                  {pendingBills.map((bill) => (
                    <div
                      ...
                      style={{
                        ...
                        marginBottom: '10px',
                        cursor: 'pointer',
                        border: selectedBill?.id === bill.id ? '2px solid #10b981' : '1px solid transparent',
                        transition: 'all 0.2s',
                      }}
                            ...
                            <tr key={item.id}>
                              ...
                              <td style={{ padding: '10px', borderBottom: '1px solid #374151' }}>
                                {item.item_status === 'pending' && (
                                  <button
                                    style={{ backgroundColor: '#10b981', color: '#fff', border: 'none', padding: '4px 8px', borderRadius: '4px', cursor: 'pointer', fontSize: '12px' }}
                  ...
                  <tr key={item.id}>
                    ...
                    <td style={styles.td}>
                      <span style={{ 
                        ...
                        fontWeight: getExpiryStatus(item.expiryDate).weight || 'normal'
                      }}>
                        {item.expiryDate || '-'} {getExpiryStatus(item.expiryDate).label}
                      </span>
                    </td>
                    ...
                    <td style={{...styles.td, fontWeight: '600', color: item.quantity <= 10 ? '#ef4444' : '#10b981'}}>
                      {item.quantity || 0}
                    </td>
