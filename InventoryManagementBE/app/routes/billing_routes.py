...
def generate_unique_bill_number():
    ...
    while True:
        # Format: BT-YYMMDD-XXXXXXXX (BT = Bill Transaction)
        now = datetime.now()
        year = str(now.year)[-2:]
...
def search_products_for_billing():
    ...
    try:
        ...
        result = [{
            ...
            'expiryDate': p.expiry_date or '',
            'hsnCode': p.hsn_code or '',
            'gstRate': p.gst_rate or 0.0,
...
def get_product_by_barcode(barcode):
    ...
    try:
        ...
        return jsonify({
            ...
            'batchNo': product.batch_no or '',
            'expiryDate': product.expiry_date or '',
            'hsnCode': product.hsn_code or '',
            'gstRate': product.gst_rate or 0.0,
...
def create_bill():
    ...
    try:
        ...
        bill.doctor_name = data.get('doctorName', '')
        bill.patient_age = data.get('patientAge', None)
        bill.patient_gender = data.get('patientGender', '')
...
# ------------------ GET ALL BILL ITEMS (for Stock Out) ------------------
@billing_bp.route("/billing/items", methods=["GET"])
def get_all_bill_items():
    """Get all items across all bills with bill info"""
    try:
        # Join BillItem with Bill to get bill info
        ...
        print(f"Get all items error: {str(e)}")
        print(traceback.format_exc())
        return jsonify({"error": "Failed to fetch bill items"}), 400
