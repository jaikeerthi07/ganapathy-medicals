...
def update_product(id):
    try:
        ...
        if data.get('batchNo') is not None:
            product.batch_no = data['batchNo'].strip()
        if data.get('expiryDate') is not None:
            product.expiry_date = data['expiryDate'].strip()
        ...
        if data.get('gstRate') is not None:
            product.gst_rate = float(data['gstRate'])
        if data.get('category') is not None:
            product.category = data['category'].strip()
        if data.get('composition') is not None:
            product.composition = data['composition'].strip()
        ...
        for idx, product_data in enumerate(products):
            try:
                ...
                product = Product(
                    ...
                    batch_no=product_data.get("batchNo", "").strip(),
                    expiry_date=product_data.get("expiryDate", "").strip(),
                    manufacturer=product_data.get("manufacturer", "").strip(),
                    hsn_code=product_data.get("hsnCode", "").strip(),
                    gst_rate=float(product_data.get("gstRate", 0.0)),
                    category=product_data.get("category", "").strip(),
                    composition=product_data.get("composition", "").strip(),
