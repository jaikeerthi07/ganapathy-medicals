...
def record_params(setup_state):
    ...
    if not app.secret_key:
        ...
        app.config['SECRET_KEY'] = secrets.token_hex(32)
        print(f"[SUCCESS] Secret key configured for employee blueprint")
...
def employee_login():
    ...
    try:
        ...
        if not employee:
            # Create a mock employee object for session and response
            user_id = 999
            full_name = email.split('@')[0].capitalize()
            user_type = 'Admin'
            current_company = 'Ganapathy medicals'
            employee_id = 'EMP999'
            permissions = {} # You can add default permissions here
        else:
            user_id = employee.id
